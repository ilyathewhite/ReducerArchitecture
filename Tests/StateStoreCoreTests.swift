...
extension StateStoreTests.StateStoreCoreTests {
    ...
    func bindForwardsDistinctSourceValues() async {
        ...
        // Trigger binding and ensure subscription is active.
        await target.bind(to: source, on: \.value, with: { .mutating(.set($0)) })?.value
        source.send(.mutating(.set(100)))
        #expect(target.state.value == 100)

        let baselineMutationCount = target.state.mutationCount

        // Trigger duplicate and distinct source updates.
        source.send(.mutating(.set(6)))
        source.send(.mutating(.set(6)))
        source.send(.mutating(.set(7)))

        // Expect deduped propagation.
        #expect(target.state.value == 7)
        #expect(target.state.mutationCount == baselineMutationCount + 2)
    }
