...
extension StateStoreTests.StateStoreAdvancedTests {
    ...
    func publisherEffectStopsAfterStoreCancel() async {
        ...
        let store = EffectHarnessNsp.store(publisher: publisher)

        // Trigger subscription, observe first value, then cancel.
    ...
    func bindPublishedValueForwardsSourceValues() async {
        ...
        // Trigger binding and first publish.
        await target.bindPublishedValue(of: source, with: { .mutating(.append($0)) })?.value
        source.publish(21)

        // Expect forwarded target value.
        #expect(target.state.values == [21])
    }
    ...
    func bindPublishedValueCancelsTargetOnSourceCancel() async {
        ...
        // Trigger binding and source cancellation.
        target.bindPublishedValue(of: source, with: { .mutating(.append($0)) })
        await Task.yield()
        source.cancel()
        for _ in 0..<50 where !target.isCancelled {
            await Task.yield()
        }
...
extension LifecycleTests.StateStoreLifecycleTests {
    ...
    func storeLifecycleLogTracksEventsForStoreLifetime() async throws {
        ...
        guard let trackedID else {
            #expect(Bool(false))
            return
        }
        #expect(storeLifecycleLog.lastEvent[trackedID] == nil)
    }
}
